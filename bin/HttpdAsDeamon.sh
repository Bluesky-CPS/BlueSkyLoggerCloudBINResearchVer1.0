...
if [ ! "$1" ]; then 
	...
	if [ ! -f "$1" ]; then 
	    ...
	    sudo su -c "nohup java -Djava.library.path=$(pwd)/SkyBlue/LCP/EDLoggingControl/LogDecompressor -cp .:lib/sqlite-jdbc-3.7.2.jar -classpath Httpd.jar:lib/sqlite-jdbc-3.7.2.jar SkyBlue.LCP.Httpd.core.Httpd -conf $1 -deamon >> /dev/null 2>&1 &"
	fi
fi
